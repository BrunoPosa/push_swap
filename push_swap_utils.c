...
/*
	...
	Reverse rotates if the rotate_one gets 'r' as 1st argument, and just rotates if anything else e.g. ascii 'a' + ('r' - 'a') = 'r'.
...
int	find_midvalue(struct s_stack *stack)
{
	...
	while (i != stack->top)
	{
		...
		if (smaller_values == (stack->top - 1) / 2) //  better to have less pb + pa operations, so sending less values to b is better instead if == stack.top / 2
